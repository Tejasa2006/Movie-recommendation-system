...
l=tk. Label(root,text='Email',bg="black",fg="white",font=('Cambria',14)).place(x=600,y=395)
new_password_entry=tk.Entry(root,width=40,textvariable=email).place(x=800,y=400)

l=tk. Label(root,text='New Password',bg="black",fg="white",font=('Cambria',14)).place(x=600,y=445)
new_password_entry=tk.Entry(root,width=40,textvariable=password).place(x=800,y=450)

l=tk. Label(root,text='Confirm Password',bg="black",fg="white",font=('Cambria',14,)).place(x=600,y=495)
confirm_password_entry=tk.Entry(root,width=40,show="*",textvariable=confirmPassword).place(x=800,y=500)
...
def change_password():
    ...
    find_user = ('SELECT * FROM DataReg WHERE Email=?')
    ...
    if result:
        if new_password_entry == confirm_password_entry:
            ...
            curs.execute("update DataReg set password=? WHERE Email=? ",(str(new_password_entry),email.get()))
...
b=tk.Button(root,text="Send",width=10,command=change_password,bg="#27ae60")
